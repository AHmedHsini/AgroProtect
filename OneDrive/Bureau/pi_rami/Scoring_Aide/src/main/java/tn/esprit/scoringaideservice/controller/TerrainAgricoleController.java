...
public class TerrainAgricoleController {
    ...
    public TerrainAgricole update(@PathVariable Long id,
        ...
        t.setLatitude(terrain.getLatitude());
        t.setLongitude(terrain.getLongitude());
        t.setGeometryJson(terrain.getGeometryJson());
        t.setEosFieldId(terrain.getEosFieldId());

        // 🔥 IMPORTANT (agriculteur)
        if (terrain.getAgriculteur() != null) {
            t.setAgriculteur(terrain.getAgriculteur());
        }
