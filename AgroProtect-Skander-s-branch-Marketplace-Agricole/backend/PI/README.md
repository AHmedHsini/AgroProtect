# AgroProtect Application

Production-grade User & Identity application for AgroProtect - Esprit University.
...
```sql
CREATE DATABASE agroprotect_db;
```
...
```
AgroProtect/
├── src/main/java/tn/esprit/agroprotect/identity/
...
java -jar target/AgroProtect-1.0.0-SNAPSHOT.jar
```
...
```bash
# Build image
docker build -t agroprotect .

# Run
docker run -p 8080:8080 --env-file .env agroprotect
```

## 📝 License

Proprietary - AgroProtect Esprit University
