...
public class QuoteController {
    ...
    public ResponseEntity<byte[]> generatePdf(
        ...
        // ✅ FIX ICI
        QuoteResponse bestResponse = quoteService.selectBestQuote(
                quote.getProject().getIdProject()
        );

        Quote bestQuote = bestResponse.getQuote();

        byte[] pdf = pdfService.generateQuotePdf(
                quote, score, success, reason, baseUrl, bestQuote
        );
